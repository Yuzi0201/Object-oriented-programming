//作业：修改payment，用基类指针实现对派生类各对象的操作
#include <iostream>
using namespace std;
...
public:
	...
	void print(normal *) { cout << "This worker's salary is:" << salary << endl << endl << endl; }
protected:
	int basic;
	int salary;
};
...
int main()
{
	...
	normal A; saler B; ceo C; normal *a = &A; saler *b = &B; ceo *c = &C;
	while (true)
	{
		...
		switch (chooser)
		{
		case 1:A.input(); A.pay(); A.print(a); break;
		case 2:B.salerinput(); B.pay(); B.print(b); break;
		case 3:C.ceoinput(); C.pay(); C.print(c); break;
